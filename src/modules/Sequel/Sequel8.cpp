...
struct Sequel8 : Module
{
	...
	void rotateRowLeft(short row)
	{
		...
		params[getKnobId(row, 7)].setValue(tempKnob);
		params[getButtonId(row, 7)].setValue(tempGate);
	}

	void rotateRowRight(short row)
	{
		float tempKnob = params[getKnobId(row, 7)].getValue();
		float tempGate = params[getButtonId(row, 7)].getValue();
